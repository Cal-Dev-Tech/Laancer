...
class OrderController extends Controller
{
                ...
                ->where(function ($query) {
                    $query->whereHas('freelancer', function ($q) {
                        ...
                        ->orWhere(function ($subQuery) {
                            $subQuery->whereHas('freelancer')->where('payment_gateway', 'manual_payment')
    ...
    {
        $client_id = Auth::guard('web')->user()->id;
        $order_details = Order::with(['freelancer:id,first_name,last_name,email,phone,country_id,state_id,city_id,image,username,user_verified_status','order_submit_history'])
            ->where('id',$id)
    ...
    public function request_revision(Request $request)
    {
        ...
        event(new ProjectEvent(__('Request for revision'), $freelancer->freelancer_id));
    ...
    public function order_milestone_approve($id, $type)
    {
        if($type == 'milestone'){
            ...
            $freelancer_wallet = Wallet::firstOrCreate(
                ['user_id' => $freelancer_id->freelancer_id],
            ...
            WalletHistory::create([
                ...
                'transaction_id' => 'order_' . $milestone->id . '_' . time(),
                'status' => 1,
            ...
            notificationToAdmin($milestone->order_id, Auth::user()->id,'Order','Order accepted by client');
            event(new ProjectEvent(__('Order accepted by client'), $freelancer_id->freelancer_id));
            ...
            if($next_milestone){
              ...
              if($order->status == 3){
                  return back()->with(toastr_warning(__('Order already accepted')));
              }
            ...
            if($order->is_fixed_hourly == 'hourly'){
                ...
                $subscription_commission = get_user_subscription_commission($order->freelancer_id);

                // Updated function call
                $commission_amount = commission_amount($amount, $individual_commission, $subscription_commission, $commission_type, $commission_charge);
                ...
                if($client_wallet->balance >= $amount){
                    ...
                    WalletHistory::create([
                        ...
                        'transaction_fee' => 0,
                        'total' => $amount,
                        'transaction_id' => 'order_' . $order->id . '_' . time(),
                        'status' => 1,
                        'currency' => get_static_option('site_global_currency'),
                        'symbol' => get_static_option('site_global_currency'),
                        'type' => 'earning'
            ...
            event(new ProjectEvent(__('Order accepted by client'), $order->freelancer_id));
