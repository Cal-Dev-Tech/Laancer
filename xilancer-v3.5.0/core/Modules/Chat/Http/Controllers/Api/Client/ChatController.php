    ...
    public function message_send(Request $request)
    {
        ...
        if($order_details?->is_project_job != 'offer'){
            ...
            if(get_static_option('profile_switch_enable_disable') == 'enable'){
                $find_freelancer = User::where('id',$request->freelancer_id)->first();
            }else{
                $find_freelancer = User::where('id',$request->freelancer_id)->where('user_type',2)->first();
            }

            if(auth('sanctum')->id() == $request->freelancer_id){
                return response()->json(['msg'=> __('Message send failed')])->setStatusCode('422');
            }
