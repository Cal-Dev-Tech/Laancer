<script>
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            function jobs(page,country,skill,type,level,min_price,max_price,duration,job_search_string){
                $.ajax({
                    url:"{{ route('skill.jobs.pagination').'?page='}}" + page,
                    method:'GET',
            ...
            $(document).on('click', '#category_job_filter_reset', function(){
                ...
                let skill = $('#skill_id').val();
                $.ajax({
                    url:"{{ route('skill.jobs.filter.reset')}}",
                    method:'GET',
                    data:{skill:skill},
                    success:function(res){
                        if(res.status=='nothing'){
