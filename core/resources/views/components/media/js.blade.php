...
@php
    $type = $type ?? 'admin';
    $trash_icon =  $type === 'admin' ? 'fas fa-times' : 'fas fa-times';
    $check_icon =  $type === 'admin' ? 'fas fa-check' : 'fas fa-check';
...
<script>
    (function ($) {
                ...
                success: function (data) {
                    ...
                    $.each(data,function (index,value) {
                        ...
                        if($('.media-uploader-image-list li[data-imgid="'+value.image_id+'"]').length < 1){
                            ...
                            var fileExtension = value.img_url.split('.').pop().toLowerCase();
                            ...
                            if (['mp4'].includes(fileExtension)) {
                                mediaTag = '<video controls>\n' +
                                    '<source src="'+value.img_url+'" type="video/'+fileExtension+'">\n' +
                                    '</video>';
                            } else {
                                mediaTag = '<img src="'+value.img_url+'" alt="'+value.alt+'">';
                            }
        ...
        $(document).on('click','#loadmorewrap',function (){
            ...
            $('#loadmorewrap button').append(' <i class="fas fa-spinner fa-spin"></i>'); //la spinner
            $.ajax({
                type: "POST",
