    ...
    public function edit_profile_photo(Request $request)
    {
        ...
        if (cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi'])) {
            if ($image = $request->file('image')) {
                $image_validation = moduleExists('CurrencySwitcher')
                    ? 'required|mimes:jpg,jpeg,png,gif,svg|max:20480'
                    : 'required|mimes:jpg,jpeg,png,gif,svg|max:1024';
    ...
    public function identity_verification(Request $request)
    {
        ...
        if($request->isMethod('post')){
            ...
            $request->validate([
                ...
                'front_image' => 'required|image|mimes:jpeg,png,jpg|max:5120|dimensions:width=2000,height=2000',
                'back_image' => 'required|image|mimes:jpeg,png,jpg|max:5120|dimensions:width=2000,height=2000',
            ],[
                'front_image.dimensions' => 'Front image must not exceed 2000×2000 pixels.',
                'back_image.dimensions' => 'Back image must not exceed 2000×2000 pixels.',
            ]);
