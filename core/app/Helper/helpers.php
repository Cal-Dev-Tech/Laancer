...
function get_attachment_url_only($id, $size = null, $default = false)
{
    $image_details = MediaUpload::find($id);

    if (!$image_details) {
        return $default ? asset('assets/uploads/no-image.png') : '';
    }

    $base_path = 'assets/uploads/media-uploader/';
    $path_suffix = $size ? $size . '-' : '';
    $image_path = $base_path . $path_suffix . $image_details->path;

    // Check if the specified size image exists, else fallback to the original
    if (file_exists($image_path)) {
        return asset($image_path);
    }

    // Fallback to original size if specific size is not available
    if (file_exists($base_path . $image_details->path)) {
        return asset($base_path . $image_details->path);
    }

    // Return default image if set to true
    return $default ? asset('assets/uploads/no-image.png') : '';
}
...
function site_currency_symbol($text = false)
{
    ...
    if (moduleExists('CurrencySwitcher')) {
        if (!empty(Session::get('user_current_currency'))) {
            $get_user_currency = \Modules\CurrencySwitcher\App\Models\SelectedCurrencyList::where('currency', Session::get('user_current_currency'))->where('status', 1)->first();
            foreach ($all_currency as $currency => $sym) {
                if ($get_user_currency->currency == $currency) {
                    $symbol = $text ? $currency : $sym;
                    break;
                }
            }
            ...
            // GeoIP reader to detect country
            $reader = new \GeoIp2\Database\Reader(public_path() . '/GeoLite2-Country.mmdb');
            $ip_address = request()->ip() == '127.0.0.1' ? '221.120.227.235' : request()->ip();
            $record = $reader->country($ip_address);
            $country = $record->country->name;

            if ($country) {
                $currency_symbol = get_currency_symbol_for_symbol($country);
                $get_user_currency = \Modules\CurrencySwitcher\App\Models\SelectedCurrencyList::where('symbol', $currency_symbol)
                    ->where('status', 1)
                    ->first();
...
function render_payment_gateway_for_form()
{
    ...
    $all_gateway = ['paypal','manual_payment','mollie','paytm','stripe','razorpay','flutterwave','paystack','marcadopago','instamojo','cashfree','payfast','midtrans','squareup','cinetpay','paytabs','billplz','zitopay','sslcommerce_gateway'];
...
function render_drag_drop_form_builder_field_markup($key, $type, $name, $placeholder, $required, $args = [])
{
    ...
    if ($type == 'file') {
        ...
        $output .= '<option value="mimes:mp4"';
        if (isset($args['mimes_type']) && $args['mimes_type'] == 'mimes:mp4') {
            $output .= "selected";
        }
        $output .= '>' . __('mimes:mp4') . '</option>';
...
function render_draggable_menu_by_id($id)
{
    ...
    $mega_menu_items = '';
    $output = '';
    $menu_details_from_db = App\Models\Menu::find($id);
...
if (!function_exists('get_currency_symbol_for_symbol')) {
    function get_currency_symbol_for_symbol($country_name)
    {
        // Path to your currency JSON file
        $file_path = public_path('currency.json');

        // Check if the file exists
        if (file_exists($file_path)) {
            // Read and decode the JSON file
            $currency_data = json_decode(file_get_contents($file_path), true);

            // Check if the country exists in the data and return the symbol
            if (isset($currency_data[$country_name])) {
                return $currency_data[$country_name]['symbol'];
            }
        }
...
function float_amount_with_currency_symbol($amount, $text = false)
{
    ...
    if (moduleExists('CurrencySwitcher')) {
        if (!empty(Session::get('user_current_currency'))) {
            $get_user_currency = \Modules\CurrencySwitcher\App\Models\SelectedCurrencyList::where('currency', Session::get('user_current_currency'))
                ->where('status', 1)
                ->first();
            $amount = (float)$amount * $get_user_currency?->conversion_rate;
        } else {
            $reader = new \GeoIp2\Database\Reader(public_path() . '/GeoLite2-Country.mmdb');
            $ip_address = request()->ip() == '127.0.0.1' ? '221.120.227.235' : request()->ip();
            $record = $reader->country($ip_address);
    ...
    $decimal_yes_or_no = get_static_option('enable_disable_decimal_point');
    $amount = $decimal_yes_or_no != 'disable' ? $amount = number_format((float)$amount, 2, $decimal_separator, $thousand_separator) : $amount = number_format((int) $amount, 0, '', $thousand_separator);
    $return_val = $symbol . $amount;
...
function float_amount_without_currency_symbol($amount, $text = false)
{
    if (moduleExists('CurrencySwitcher')) {
        if (!empty(Session::get('user_current_currency'))) {
            $get_user_currency = \Modules\CurrencySwitcher\App\Models\SelectedCurrencyList::where('currency', Session::get('user_current_currency'))
            ...
            $currency_symbol = get_currency_symbol_for_symbol($country);
            $get_user_currency = \Modules\CurrencySwitcher\App\Models\SelectedCurrencyList::where('symbol', $currency_symbol)->where('status', 1)->first();
            if($get_user_currency){
                $amount = (float)$amount * $get_user_currency?->conversion_rate;
            }
        }
    ...
    //decimal enable disable
    $amount = !is_null($amount) ? (float)$amount : $amount;
    $return_val = $amount;
    return $return_val;
}
...
function render_site_meta($title){
...
function render_site_meta_home(){
    ...
    return <<<HTML
<!-- Primary Meta Tags -->
<meta name="title" content="{$site_title} - {$site_tag_line}">
<meta name="description" content="{$site_desc}">
<meta name="keywords" content="{$site_tags}">
...
function render_page_meta_data_for_service($service_details){
    ...
    $user_lang = LanguageHelper::user_lang_slug();
    $site_url = route('project.details',
        [
            'username' => $service_details->project_creator?->username,
            'slug' => $service_details->slug]);

    $description= cleanText($service_details?->description);

    $meta_title =  $service_details->metaData?->meta_title ?? $service_details?->title;
    $site_tags = $service_details->metaData?->meta_tags ?? $service_details?->title;
    $site_description =  $service_details->metaData?->meta_description ?? $description;

    //facebook
    $facebook_meta_tags = $service_details->metaData?->facebook_meta_tags ?? $meta_title;
    $facebook_meta_description =  $service_details->metaData?->facebook_meta_description ?? $site_description;
    $facebook_meta_image = !empty($service_details->metaData->facebook_meta_image)
        ? $service_details->metaData->facebook_meta_image
        : get_attachment_image_by_id($service_details?->image)['img_url'] ?? '';

    $facebook_meta_image_alt = !empty($service_details->metaData->facebook_meta_image)
        ? $service_details->metaData->facebook_meta_image
        : get_attachment_image_by_id($service_details?->image)['img_url'] ?? '';

    $twitter_meta_tags = $service_details->metaData?->twitter_meta_tags ?? $meta_title;
    $twitter_meta_description =  $service_details->metaData?->twitter_meta_description ?? $site_description;
    $twitter_meta_image = !empty($service_details->metaData->twitter_meta_image)
...
function render_page_meta_data_for_job($job_details){
    ...
    $user_lang = LanguageHelper::user_lang_slug();
    $site_url = route('job.details',
        [
            'username' => $job_details->job_creator?->username,
            'slug' => $job_details->slug]);

    $description= cleanText($job_details?->description);

    $meta_title =  $job_details->metaData?->meta_title ?? $job_details?->title;
    $site_tags = $job_details->metaData?->meta_tags ?? $job_details?->title;
    $site_description =  $job_details->metaData?->meta_description ?? $description;

    //facebook
    $facebook_meta_tags = $job_details->metaData?->facebook_meta_tags ?? $meta_title;
    $facebook_meta_description =  $job_details->metaData?->facebook_meta_description ?? $site_description;
    $facebook_meta_image = !empty($job_details->metaData->facebook_meta_image)
        ? $job_details->metaData->facebook_meta_image
        : get_attachment_image_by_id($job_details?->image)['img_url'] ?? '';

    $facebook_meta_image_alt = !empty($job_details->metaData->facebook_meta_image)
        ? $job_details->metaData->facebook_meta_image
        : get_attachment_image_by_id($job_details?->image)['img_url'] ?? '';

    $twitter_meta_tags = $job_details->metaData?->twitter_meta_tags ?? $meta_title;
    $twitter_meta_description =  $job_details->metaData?->twitter_meta_description ?? $site_description;
    $twitter_meta_image = !empty($job_details->metaData->twitter_meta_image)
        ? $job_details->metaData->twitter_meta_image
        : get_attachment_image_by_id($job_details?->image)['img_url'] ?? '';
...
//         $commission_amount = $individual_commission->admin_commission_type == 'fixed' ? $individual_commission->admin_commission_charge : ((float)$price*$individual_commission->admin_commission_charge/100);
//     }else{
//         $commission_amount = $commission_type == 'fixed' ? $commission_charge : ((float)$price*$commission_charge/100);
//     }
//     return $commission_amount;
// }

function commission_amount($price, $individual_commission, $subscription_commission, $commission_type, $commission_charge)
{
    // Individual commission (highest priority)
    if ($individual_commission
        && $individual_commission->admin_commission_type
        && $individual_commission->admin_commission_charge !== null) {
        $commission_amount = $individual_commission->admin_commission_type === 'fixed' 
            ? $individual_commission->admin_commission_charge 
            : ((float)$price * $individual_commission->admin_commission_charge / 100);
        return $commission_amount;
    }
    
    // Subscription commission (medium priority)
    if ($subscription_commission && isset($subscription_commission['type'], $subscription_commission['rate'])) {
        return $subscription_commission['type'] === 'fixed'
            ? $subscription_commission['rate']
            : ((float)$price * $subscription_commission['rate'] / 100);
    }
    
    // Global commission (lowest priority - fallback)
    $commission_amount = $commission_type == 'fixed' 
        ? $commission_charge 
        : ((float)$price * $commission_charge / 100);
    ...
    if ($userSubscription && $userSubscription->subscription) {
        ...
        if ($subscription->commission_rate !== null && $subscription->commission_type !== null) {
            return [
                ...
                'rate' => $subscription->commission_rate
            ];
        }
    }
...
//                ->withNotification(Notification::create($last_notification->type, $last_notification->message))
                ->withData($notificationBody);
            $response = $messaging->sendMulticast($message, $freelancer?->firebase_device_token);
        }
    }catch (\Exception $e){}
}
...
//                ->withNotification(Notification::create($last_notification->type, $last_notification->message))
                ->withData($notificationBody);
            $response = $messaging->sendMulticast($message,$client_for_device?->firebase_device_token );
        }
    }catch (\Exception $e){}
}
...
function project_rating2($project_id)
{
...
{
    // Get the total completed orders count and total ratings sum in one query
    $result = Order::join('ratings', 'ratings.order_id', '=', 'orders.id')
        ->where('orders.identity', $project_id)
        ->where('orders.status', 3)
        ->where('ratings.sender_type', 1) // Ensuring we only count sender_type 1 ratings
        ->selectRaw('COUNT(DISTINCT orders.id) as total_orders, SUM(ratings.rating) as total_rating')
        ->first();

    $project_complete_order_count_2 = Order::where('identity', $project_id)
        ->where('status', 3)
        ->count();

    $project_complete_order_count = $result->total_orders ?? 0;
    $total_rating = $result->total_rating ?? 0;

    // Calculate the average rating
    $avg_rating = $project_complete_order_count > 0 ? $total_rating / $project_complete_order_count : 0;

    // Generate the string based on the project_complete_order_count and avg_rating
    if ($project_complete_order_count >= 1 && $avg_rating >= 1) {
        $string = '<div class="single-project-content-review">
                        <span class="single-project-content-review-icon"> <i class="fa-solid fa-star"></i> </span>
                        <span class="single-project-content-review-rating">'.round($avg_rating, 1).'('.$project_complete_order_count.') </span>
                   </div>
                     <a href="#/" class="single-project-orderCompleted"> '.$project_complete_order_count_2.' '.__("Orders Completed").' </a>';
    } else if ($project_complete_order_count >= 1) {
        $string = '<div class="single-project-content-review">
                        <span class="single-project-content-review-rating">'.__("No Review").'</span>
                   </div>
    ...
    if($header == null){
        if($complete_orders_count >= 1 && $avg_rating >= 1){
            $string = '<div class="jobFilter-proposal-author-contents-review-flex">
                        <span class="jobFilter-proposal-author-contents-review-icon"><i class="fas fa-star"></i></span>
...
function freelancer_rating_for_profile_details_page2($freelancer_id)
{
...
{
    // Get the complete orders
    $complete_orders = Order::select(['id','identity','status'])
        ->where('freelancer_id', $freelancer_id)
        ->where('status', 3)
        ->get();

    // Get all ratings related to the completed orders in one query
    $ratings = Rating::whereIn('order_id', $complete_orders->pluck('id'))
        ->where('sender_type', 1)
        ->get();
    ...
    if ($complete_orders->count() >= 1 && $rating_count >= 1) {
        return '
...
function freelancer_level2($freelancer_id,$call_from_telent_page = null)
{
...
{
    // Fetch levels with their rules in a single query
    $levels = \Modules\FreelancerLevel\Entities\FreelancerLevel::with('level_rule')
        ->whereHas('level_rule')
        ->where('status', 1)
        ->get();
    ...
    // Fetch freelancer details in a single query
    $freelancer_details = \App\Models\User::select('id', 'created_at')
        ->where('id', $freelancer_id)
        ->first();

    // Calculate the difference in days
    $diff_in_days = $freelancer_details->created_at->diffInDays(now());

    // Fetch freelancer criteria in a single query
    $freelancer_criteria = Order::where('freelancer_id', $freelancer_id)
        ->where('status', 3)
        ->selectRaw('COUNT(id) as total_order, SUM(payable_amount) as total_earnings')
        ->first();

    $total_order = $freelancer_criteria->total_order ?? 0;
    $total_earnings = $freelancer_criteria->total_earnings ?? 0;
    $avg_rating = freelancer_rating_for_level($freelancer_id);


    // Determine the freelancer level based on rules
    foreach ($levels as $level) {
        $period = $level->level_rule->period;
        $complete_order = $level->level_rule->complete_order ?? 0;
        $earning = $level->level_rule->earning ?? 0;
        $rule_avg_rating = $level->level_rule->avg_rating ?? 0;

        // Check if the freelancer meets the criteria for the current level
        if (
            $total_order >= $complete_order &&
            $total_earnings >= $earning &&
            $avg_rating >= $rule_avg_rating
        ) {
            // Check the period range
            if (
                ($period >= 1 && $period < 3 && $diff_in_days >= 30 && $diff_in_days < 90) ||
                ($period >= 3 && $period < 6 && $diff_in_days >= 90 && $diff_in_days < 180) ||
                ($period >= 6 && $period < 9 && $diff_in_days >= 180 && $diff_in_days < 270) ||
                ($period >= 9 && $period < 12 && $diff_in_days >= 270 && $diff_in_days < 360) ||
                ($period >= 12 && $diff_in_days >= 360)
            ) {
                // Render the level badge based on the calling context
                if ($call_from_telent_page == 'talent') {
                    $freelancer_level = '<div class="level-badge-wrapper">' . render_image_markup_by_attachment_id($level->image) . ' ' . '<span class="badge-title">' . $level->level . '</span></div>';
                } else {
                    $freelancer_level = '(' . $level->level . ')';
                }
                break; // Exit the loop once the correct level is found
            }
        }
    }
...
function freelancer_level_api($freelancer_id,$call_from_telent_page = null)
{
    if(moduleExists('FreelancerLevel')){
        //get level with level rules
        $levels = \Modules\FreelancerLevel\Entities\FreelancerLevel::with('level_rule')
            ->whereHas('level_rule')
            ->where('status',1)
            ->get();
        ...
        foreach ($levels as $level){
            if ($level->level_rule->period >= 1 && $level->level_rule->period < 3){
                if($diff_in_days >= 30 && $diff_in_days < 90){
                    if($total_order >= $level?->level_rule?->complete_order && $total_earnings >= $level?->level_rule?->earning && $avg_rating >= $level?->level_rule?->avg_rating){
                        $freelancer_level = ucfirst($level->level);
                        $level_img = get_attachment_url_only($level->image);
                    }
                }
            }
            elseif($level->level_rule->period >= 3 && $level->level_rule->period < 6){
    ...
    return [
        'freelancer_level' => $freelancer_level ?? null,
        'level_img' => $level_img ?? null
    ];
}


//freelancer rating for level
function freelancer_rating_for_level2($freelancer_id)
{
...
{
    // Get the total count of completed orders and the sum of ratings in one query
    $result = Order::selectRaw('COUNT(DISTINCT orders.id) as total_orders, SUM(ratings.rating) as total_rating')
    ...
    $total_order = $result->total_orders ?? 0;
    $total_rating = $result->total_rating ?? 0;

    // Calculate the average rating
    $avg_rating = $total_order > 0 ? $total_rating / $total_order : 0;

    // If there's at least 1 completed order and 1 rating, return the average rating
    if ($total_order >= 1) {
        return round($avg_rating, 1);
    } else {
        return ''; // Return empty if no ratings or completed orders
    }
}
...
function get_currency_according_to_user()
{
    $currency = null;
    if (moduleExists('CurrencySwitcher')) {
        if (!empty(Session::get('user_current_currency'))) {
...
function get_conversion_price($price)
{
    ...
    if (moduleExists('CurrencySwitcher')) {
        if (!empty(Session::get('user_current_currency'))) {
            ...
            $get_user_currency = \Modules\CurrencySwitcher\App\Models\SelectedCurrencyList::where('currency', Session::get('user_current_currency'))
                ->where('status', 1)
                ->first();
            $conversion_price = $get_user_currency->conversion_rate * $price;
        } else {
            // GeoIP reader to detect country
            $reader = new \GeoIp2\Database\Reader(public_path() . '/GeoLite2-Country.mmdb');
            $ip_address = request()->ip() == '127.0.0.1' ? '221.120.227.235' : request()->ip();
...
function convertDbCurrencyWithLocalCurrency(float|int|null $amount): int|float|null
{
    if(is_null($amount)){
        return null;
    }
    ...
    // check currency module exists or not if exist then call get conversion rate
    if(moduleExists('CurrencySwitcher')){
        $get_user_currency = SelectedCurrencyList::where('currency', get_currency_according_to_user())->first() ?? null;
        $latestAmount = $amount * $get_user_currency->conversion_rate;
    }

    return $latestAmount;
}

function render_page_meta_data_for_profile_details_title($user){
    $user_name=$user->first_name." ".$user->last_name;
    $meta_title = $user_name ??  __('Profile Details');
    return $meta_title;
}

if (!function_exists('unread_deposit_count')) {
    function unread_deposit_count()
    {
        return AdminNotification::unreadCount('Deposit Amount');
    }
}

if (!function_exists('unread_withdraw_count')) {
    function unread_withdraw_count()
    {
        return AdminNotification::unreadCount('Withdraw');
    }
}

if (!function_exists('skillToSlug')) {
    function skillToSlug($skillName) {
        $slug = preg_replace('/[^A-Za-z0-9]+/', '-', $skillName);
